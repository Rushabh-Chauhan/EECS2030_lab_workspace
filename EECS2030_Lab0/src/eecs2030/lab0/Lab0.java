...
public class Lab0 {
	...
	public static double windChill(double airTemp, double windSpeed) {
		...
		return 13.12+(0.6215*airTemp)+((0.3965*airTemp)-11.37)*wind;
	}
	...
	public static int compareTo(Range r1, Range r2) {
		double s1 = r1.getMaximum()-r1.getMinimum();
		double s2 = r2.getMaximum()-r2.getMinimum();
		int a = 1;
		if(s1 == s2)
		{
			a = 0;
		...
		else
		{
			...
			for (int i = 0; i < s.length(); i++)
			{
				if(i%2==0)
				{
					...
					sum = sum + Character.toLowerCase(s.charAt(i));
				}
				else 
				{
